...
static VkResult InitializeDevice(VkQueueFlagBits queueFlag, VkPhysicalDeviceMemoryProperties* pMemoryProperties)
{
    ...
    {
        // Here, just determine whether both VK_KHR_8bit_storage and  feature is supported.
...
static void RunDescriptorAliasingComputeTest(void)
{
    puts("\n================ Begin Descriptor-Aliasing compute test ================\n");
    ...
    do
    {
        ...
        int* testBuffer = calloc(elemCount, sizeof(*testBuffer));
        for (int i = 0; i < (int)elemCount; ++i) {
            testBuffer[i] = i;
        }
        const uint8_t* test8BitBuffer = (uint8_t*)testBuffer;

        for (int i = 0; i < (int)elemCount; i++)
        {
            if (dstMem[i] != testBuffer[i] + (int)test8BitBuffer[i])
            {
    ...
    puts("\n================ Complete Descriptor-Aliasing compute test ================\n");
}

static void RunArrayRefComputeTest(void)
{
    puts("\n================ Begin Array Reference compute test ================\n");
    ...
    do
    {
        ...
        result = CreateShaderModule(s_specDevice, "shaders/array_ref.spv", &computeShaderModule);
        if (result != VK_SUCCESS)
        {
            fprintf(stderr, "CreateShaderModule failed!\n");
            break;
    ...
    puts("\n================ Complete Array Reference compute test ================\n");
}
...
int main(int argc, const char* argv[])
{
    ...
    {
        RunDescriptorAliasingComputeTest();
        RunArrayRefComputeTest();
    }
